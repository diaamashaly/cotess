...
if ( ! function_exists( 'kr_theme_setup' ) ) {
	function kr_theme_setup() {
		...
		load_theme_textdomain( 'abrag', KR_THEME_DIR . '/languages/' );
...
if ( ! function_exists( 'kr_enqueue_scripts' ) ) {
	function kr_enqueue_scripts() {
		...
		// Css files
		$styles = array(
			'fonts-awesome' => 'font-awesome.min.css',
			'fonts'         => 'font-kr.css',
		...
		foreach ( $scripts as $alias => $src ) {
			...
			wp_enqueue_script( $alias, KR_THEME_JS_URI . $src, array( 'jquery' ), '1.1', true );
